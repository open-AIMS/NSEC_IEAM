...
dat <- all.sim.out %>%
  mutate(est.NEC=NEC_X50.,
         ...
         est.NSEC=NSEC01.ECxsigmoidal_X50.*ECxsigmoidal + NEC_X50.*NEC3param,         
...
ggsave("figure weights.png", height=120, width=200, units="mm", bg = "white")
...
dat %>%
  ...
  #5 reps (10 trials)  10 reps (10 trials) 5 reps (20 trials)  10 reps (20 trials)
  ...
  ggplot(aes(x=endpoint, y=y)) +#, col=endpoint)) +
  geom_boxplot(aes(fill=label), outlier.shape=NA, alpha=1, colour="black")+
  scale_fill_manual(values=c("#B3AE36","#2D6480"))+
  #geom_text(data=fig_estimates_labels, aes(x=x,y=y, label=label), fontface="bold")+
  geom_hline(data = na.omit(true.vals.2), aes(yintercept = y, colour=endpoint)) +
  scale_colour_manual(values = c("#41b6c4", "#F08986", "#F02451", "#91003f")) +  
  labs(col="True endpoints") + 
  ylab("Estimated value") + 
  facet_grid(scenarios_f ~ n.treatments, scales = "free_y") +
  theme_minimal() +
...
ggsave("figure all_effects_sigmoidal.png", height=160, width=240, units="mm", bg = "white")
